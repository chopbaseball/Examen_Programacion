    ...
    if request.method == 'POST':
        for i in carrito:
            if request.user.get_username() == i.usuario:
                ...
                for codigos in segui:
                    codigo = codigo +1


                if request.user.get_username() == i.usuario:
                    se = Seguimiento()
                    se.precio = totalFinal
                    se.codigo = codigo
                    se.estado_id = 1
                    se.usuario = request.user.get_username()
                    se.save()

            
                if request.user.get_username() == i.usuario:
                    carritoeliminado = Carrito.objects.all()
...
def suscripcion(request):
    ...
    suscripcion = request.user.get_username()
...
def apiNuestra(request):
    response = requests.get('http://127.0.0.1:8000/api/Producto/').json() #para api propia

    datos = {
        'apiNuestra' : response
    }
    return render(request,'app/externas/apiNuestra.html',datos)

@login_required
def seguimiento(request):
    datosALL = Seguimiento.objects.all()
    usuarios = request.user.get_username()
    datos = {
        'datos' : datosALL,
        'usuarios' : usuarios
    }
